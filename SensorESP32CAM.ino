...
//#include <Servo.h>
#include <ESP32Servo.h>// biblioteca que incluye las funciones para el servomotor
Servo motor; // Se declara la variable motor de tipo Servo
...
int MCD=15; //Pin 15 para el ventilador (Motor de corriente directa)
...
Serial.println("Termina la conexion al broquer");
  ...
  motor.attach(12);   // put your setup code here, to run once:
}// fin del void setup ()
...
void loop() {
  ...
  //if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
   if (true) {
    ...
    t=t+9;  // Se ajusta la temperatura
    //if(t>31){
    if(t>38){ // Si la temperatura es mayor a 38 grados C
    motor.write(180);  // El servo motor abre la tapa del contenedor (180 grados)
    digitalWrite(MCD, HIGH); // Y el ventilador del motor de corriente directa comienza a girar
    delay(500); // durante medio segundo
    }
   //else if(t<=31){
  else if(t<38){ //Si la temperatura es mejor a 38 °C
   motor.write(0);  // el servomotor  cierra la tapa del contenedor y regresa a su posicion inicial (0 grados)
   digitalWrite(MCD, LOW);// Y El motor del ventilador se apaga.
   delay(500); 
  }
    ...
    //printLocalTime();
    String json = "{\"id\":\"Gustavo\",\"temp\":"+String(t)+"}";
    //String json = "{\"id\":\"Gustavo\",\"temp\":"+String(t)+",\"hum\":"+String(h)+"}";
    ...
    client.publish("codigoIoT/Mor/mqtt/flow4", char_array); 
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "codigoIoT/mqtt") {  // En caso de recibirse mensaje en el tema codigoIoT/ejemplo/mqttin
    if(messageTemp == "true"){
